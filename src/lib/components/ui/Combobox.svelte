<script lang="ts">
	...
	const inputClasses =
		'w-full px-4 py-2 pr-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-all';
</script>
...
<div
	...
	class={twMerge('flex flex-col gap-1.5', containerClass as string)}
>
	{#if label}
...
{#if isOpen && filteredOptions.length > 0}
	<div
		style={dropdownStyle}
		class="z-[9999] max-h-60 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg"
	>
		{#each filteredOptions as option}
			<button
				type="button"
				onmousedown={() => handleSelect(option)}
				class="w-full px-4 py-2 text-left text-sm transition-colors hover:bg-primary-50 {option.value ===
				value
...
{:else if isOpen && searchQuery && filteredOptions.length === 0}
	<div
		style={dropdownStyle}
		class="z-[9999] rounded-lg border border-gray-200 bg-white p-4 text-center text-sm text-gray-500 shadow-lg"
	>
		No results found
	</div>
{/if}
