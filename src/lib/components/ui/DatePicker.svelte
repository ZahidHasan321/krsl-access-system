		...
		>
			...
			{#if value && !disabled}
				<button
					type="button"
					onclick={clearDate}
					class="rounded-sm p-0.5 opacity-50 ring-offset-white transition-opacity hover:opacity-100 focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 focus:outline-none"
				>
					<X class="size-3.5" />
				</button>
			{/if}
