<script lang="ts">
	...
	async function fetchItems(q: string) {
		loading = true;
		try {
			const res = await fetch(`/api/designations?q=${encodeURIComponent(q)}&limit=100`);
			if (res.ok) {
			...
			<div class="h-[280px] w-full bg-white">
				...
				{:else if !loading && searchQuery}
					<div class="flex h-full flex-col items-center justify-center p-6 text-center">
						...
						<p class="text-sm font-bold text-slate-900">Add New</p>
						<p class="mt-1 text-[10px] text-slate-500 px-4 leading-tight uppercase font-black">
							"{searchQuery}" will be saved to master list
						</p>
					</div>
				{:else if !loading}
