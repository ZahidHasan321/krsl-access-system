...
<div class="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-12 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        ...
        <Card class="p-10 shadow-2xl shadow-slate-200/50 border-0">
            <form method="post" action="?/login" use:enhance={() => {
                ...
                <Button 
                    type="submit" 
                    class="w-full h-14 text-lg font-bold bg-primary-600 hover:bg-primary-700 active:scale-[0.98] transition-all rounded-xl shadow-lg shadow-primary-500/25 flex items-center justify-center gap-3"
                    disabled={isLoading}
                >
                    {#if isLoading}
