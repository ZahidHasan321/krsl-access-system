...
<Dialog.Root bind:open>
	<Dialog.Content class="sm:max-w-[425px]">
		...
		>
				...
				<Select.Root type="single" bind:value={boundCategoryId}>
					...
					<Select.Content class="max-h-[300px] overflow-y-auto">
						{#each rootCategories as root}
							<Select.Item value={root.id} class="text-xs font-black uppercase">
								{i18n.t(root.slug as any) || root.name}
							</Select.Item>
							{#each subCategories(root.id) as sub}
								<Select.Item value={sub.id} class="pl-6 text-sm font-medium">
			...
			<div
				class="flex items-start gap-2 rounded-lg bg-amber-50 p-3 text-xs font-medium text-amber-800"
			>
