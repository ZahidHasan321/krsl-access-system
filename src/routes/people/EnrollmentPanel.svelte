<script lang="ts">
	import { onMount, onDestroy } from 'svelte';
	import { Button } from '$lib/components/ui/button';
	import { Input } from '$lib/components/ui/input';
	import { Label } from '$lib/components/ui/label';
	import {
		Fingerprint,
		Scan,
		CreditCard,
		Wifi,
		WifiOff,
		Loader2,
		X,
		AlertTriangle,
		CheckCircle
	} from 'lucide-svelte';
	import { cn, appToast } from '$lib/utils';
	import { fade } from 'svelte/transition';

	let {
		personId,
		biometricId,
		personName,
		categorySlug,
		initialCardNo = '',
		onDone,
		onSkip
	}: {
		personId: string;
		biometricId: string;
		personName: string;
		categorySlug?: string;
		initialCardNo?: string | null;
		onDone: (method: string | null, photoUrl?: string | null) => void;
		onSkip: () => void;
	} = $props();

	type Step = 'choose' | 'card-input' | 'waiting' | 'success' | 'failed';
	let step = $state<Step>('choose');
	let deviceOnline = $state(false);
	let deviceLoading = $state(true);
	let selectedMethod = $state('');
	let cardNo = $state('');
	let failureCode = $state('');
	let eventSource: EventSource | null = null;
	let enrolledPhotoUrl = $state<string | null>(null);

	onMount(async () => {
		// Initialize state from props
		if (categorySlug === 'card') {
			step = 'card-input';
			selectedMethod = 'card';
		}
		if (initialCardNo) {
			cardNo = initialCardNo;
			
			// If we have a card already from registration, auto-enroll/sync it
			// to make the flow seamless like standard person enrollment
			if (categorySlug === 'card') {
				enrollCard();
			}
		}

		// Check device status
		try {
			const res = await fetch('/api/devices/status');
			const data = await res.json();
			deviceOnline = data.online;
		} catch {}
		deviceLoading = false;

		// Listen for enrollment events via SSE
		eventSource = new EventSource('/api/events');

		eventSource.addEventListener('enrollment', (e) => {
			console.log('[SSE] Received enrollment event:', e.data);
			try {
				const data = JSON.parse(e.data);
				console.log(`[SSE] Checking if ${data.personId} matches ${personId}`);
				if (data.personId === personId) {
					console.log('[SSE] Match found! Transitioning to success step.');
					if (data.photoUrl) enrolledPhotoUrl = data.photoUrl;
					step = 'success';
				}
			} catch (err) {
				console.error('[SSE] Error parsing enrollment data:', err);
			}
		});

		eventSource.addEventListener('enrollment-failed', (e) => {
			console.log('[SSE] Received enrollment-failed event:', e.data);
			try {
				const data = JSON.parse(e.data);
				if (data.personId === personId) {
					failureCode = data.returnCode || 'unknown';
					step = 'failed';
				}
			} catch {}
		});
	});

	onDestroy(() => {
		eventSource?.close();
	});

	async function startEnroll(method: 'face' | 'finger') {
		if (!deviceOnline) {
			appToast.error('Device is offline');
			return;
		}
		selectedMethod = method;
		step = 'waiting';
		try {
			await fetch('/api/enroll', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ personId, method })
			});
		} catch {
			failureCode = 'network';
			step = 'failed';
			appToast.error('Network error during enrollment');
		}
	}

	async function enrollCard() {
		if (!cardNo.trim()) return;
		selectedMethod = 'card';
		try {
			const res = await fetch('/api/enroll', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ personId, method: 'card', cardNo: cardNo.trim() })
			});
			if (res.ok) {
				step = 'success';
				return;
			} else {
				failureCode = 'card-failed';
				step = 'failed';
				appToast.error('Failed to register card');
			}
		} catch {
			failureCode = 'network';
			step = 'failed';
			appToast.error('Network error during card registration');
		}
	}

	function handleSkip() {
		// Sync user to device when skipping enrollment so the device knows about them
		fetch('/api/enroll/sync', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ personId })
		}).catch((e) => console.error('[EnrollSkip] Failed to sync device:', e));
		onSkip();
	}
</script>

<div class="space-y-6">
	{#if step === 'choose'}
		<div in:fade>
			<!-- Device status -->
			<div
				class="mb-5 flex items-center gap-2 rounded-xl p-3 {deviceOnline
					? 'border border-emerald-200 bg-emerald-50'
					: 'border border-rose-200 bg-rose-50'}"
			>
				{#if deviceLoading}
					<Loader2 size={16} class="animate-spin text-slate-400" />
					<span class="text-xs font-bold text-slate-500">Checking device...</span>
				{:else if deviceOnline}
					<Wifi size={16} class="text-emerald-600" />
					<span class="text-xs font-bold text-emerald-700">Device connected</span>
				{:else}
					<WifiOff size={16} class="text-rose-600" />
					<span class="text-xs font-bold text-rose-700">No device connected</span>
				{/if}
			</div>

			<div class="mb-6 space-y-1 text-center">
				<h3 class="text-lg font-black text-slate-900">Enroll on Device</h3>
				<p class="text-sm text-slate-500">
					Choose enrollment method for <span class="font-bold text-slate-700">{personName}</span>
				</p>
				{#if categorySlug === 'card'}
					<div class="mt-2 flex justify-center">
						<span
							class="rounded-full bg-purple-100 px-3 py-1 text-[10px] font-bold text-purple-700 uppercase tracking-widest"
							>Card Only Category</span
						>
					</div>
				{/if}
				<p class="mt-2 text-[10px] font-bold tracking-widest text-slate-400 uppercase">
					Biometric ID: {biometricId}
				</p>
			</div>

			<!-- Method cards -->
			<div class="grid grid-cols-3 gap-3">
				<button
					disabled={!deviceOnline || deviceLoading || categorySlug === 'card'}
					onclick={() => startEnroll('face')}
					class={cn(
						'group cursor-pointer rounded-2xl border-2 p-5 text-center transition-all',
						!deviceOnline || deviceLoading || categorySlug === 'card'
							? 'cursor-not-allowed border-slate-100 bg-slate-50 text-slate-300'
							: 'border-blue-100 text-slate-600 hover:border-blue-300 hover:bg-blue-50/50 hover:text-blue-700'
					)}
				>
					<div
						class="mx-auto mb-3 flex size-12 items-center justify-center rounded-xl {!deviceOnline ||
						deviceLoading ||
						categorySlug === 'card'
							? 'bg-slate-100'
							: 'bg-blue-100 group-hover:bg-blue-200'} transition-colors"
					>
						<Scan size={24} />
					</div>
					<p class="text-xs font-black tracking-wider uppercase">Face</p>
				</button>

				<button
					disabled={!deviceOnline || deviceLoading || categorySlug === 'card'}
					onclick={() => startEnroll('finger')}
					class={cn(
						'group cursor-pointer rounded-2xl border-2 p-5 text-center transition-all',
						!deviceOnline || deviceLoading || categorySlug === 'card'
							? 'cursor-not-allowed border-slate-100 bg-slate-50 text-slate-300'
							: 'border-emerald-100 text-slate-600 hover:border-emerald-300 hover:bg-emerald-50/50 hover:text-emerald-700'
					)}
				>
					<div
						class="mx-auto mb-3 flex size-12 items-center justify-center rounded-xl {!deviceOnline ||
						deviceLoading ||
						categorySlug === 'card'
							? 'bg-slate-100'
							: 'bg-emerald-100 group-hover:bg-emerald-200'} transition-colors"
					>
						<Fingerprint size={24} />
					</div>
					<p class="text-xs font-black tracking-wider uppercase">Finger</p>
				</button>

				<button
					onclick={() => (step = 'card-input')}
					class="group cursor-pointer rounded-2xl border-2 border-amber-100 p-5 text-center text-slate-600 transition-all hover:border-amber-300 hover:bg-amber-50/50 hover:text-amber-700"
				>
					<div
						class="mx-auto mb-3 flex size-12 items-center justify-center rounded-xl bg-amber-100 transition-colors group-hover:bg-amber-200"
					>
						<CreditCard size={24} />
					</div>
					<p class="text-xs font-black tracking-wider uppercase">Card</p>
				</button>
			</div>

			{#if categorySlug === 'card'}
				<p class="mt-3 text-center text-[10px] font-bold text-purple-400">
					This category only supports Card identification
				</p>
			{:else if !deviceOnline && !deviceLoading}
				<p class="mt-3 text-center text-[10px] font-bold text-rose-400">
					Face and Finger enrollment require a connected device
				</p>
			{/if}

			<Button variant="ghost" onclick={handleSkip} class="mt-4 w-full font-bold text-slate-500">
				Skip Enrollment
			</Button>
		</div>
	{:else if step === 'card-input'}
		<div class="space-y-5" in:fade>
			<div class="space-y-1 text-center">
				<div
					class="mx-auto mb-3 flex size-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-600"
				>
					<CreditCard size={28} />
				</div>
				<h3 class="text-lg font-black text-slate-900">Card Enrollment</h3>
				<p class="text-sm text-slate-500">
					Enter the card number for <span class="font-bold text-slate-700">{personName}</span>
				</p>
			</div>

			<div class="space-y-2">
				<Label
					for="enroll-cardNo"
					class="text-[10px] font-bold tracking-widest text-slate-500 uppercase">Card Number</Label
				>
				<Input
					id="enroll-cardNo"
					bind:value={cardNo}
					class="h-12 border-2 text-center text-lg font-bold tracking-wider"
					placeholder="Enter card number"
				/>
			</div>

			<div class="flex gap-3">
				{#if categorySlug !== 'card'}
					<Button
						variant="outline"
						onclick={() => {
							step = 'choose';
						}}
						class="flex-1 border-2 font-bold"
					>
						Back
					</Button>
				{/if}
				<Button disabled={!cardNo.trim()} onclick={enrollCard} class="flex-1 font-bold">
					Register Card
				</Button>
			</div>
		</div>
	{:else if step === 'waiting'}
		<div class="space-y-5 py-6 text-center" in:fade>
			<div class="mx-auto flex size-20 items-center justify-center rounded-full bg-primary-100">
				<Loader2 size={40} class="animate-spin text-primary-600" />
			</div>
			<div class="space-y-1">
				<h3 class="text-lg font-black text-slate-900">Waiting for Enrollment</h3>
				<p class="text-sm text-slate-500">
					Please complete <span class="font-bold text-primary-700 capitalize">{selectedMethod}</span
					> enrollment on the device
				</p>
				<p class="mt-2 text-[10px] font-bold tracking-widest text-slate-400 uppercase">
					{personName}
				</p>
			</div>

			<div class="rounded-xl border border-blue-100 bg-blue-50 p-3">
				<p class="text-xs font-medium text-blue-700">
					{#if selectedMethod === 'face'}
						Ask the person to look at the device camera
					{:else}
						Ask the person to place their finger on the scanner
					{/if}
				</p>
			</div>

			<Button variant="outline" onclick={handleSkip} class="border-2 font-bold text-slate-500">
				<X size={16} class="mr-1" /> Cancel
			</Button>
		</div>
	{:else if step === 'success'}
		<div class="space-y-5 py-6 text-center" in:fade>
			<div class="mx-auto flex size-20 items-center justify-center rounded-full bg-emerald-100">
				<CheckCircle size={40} class="text-emerald-600" />
			</div>
			<div class="space-y-1">
				<h3 class="text-lg font-black text-slate-900">Enrollment Successful!</h3>
				<p class="text-sm text-slate-500">
					<span class="font-bold text-emerald-700 capitalize">{selectedMethod}</span> enrolled for
					<span class="font-bold text-slate-700">{personName}</span>
				</p>
				<p class="mt-2 text-[10px] font-bold tracking-widest text-slate-400 uppercase">
					Biometric ID: {biometricId}
				</p>
			</div>

			<Button
				onclick={() => onDone(selectedMethod, enrolledPhotoUrl)}
				class="h-12 w-full bg-emerald-600 text-sm font-black tracking-widest uppercase hover:bg-emerald-700"
			>
				Continue
			</Button>
		</div>
	{:else if step === 'failed'}
		<div class="space-y-5 py-6 text-center" in:fade>
			<div class="mx-auto flex size-20 items-center justify-center rounded-full bg-rose-100">
				<AlertTriangle size={40} class="text-rose-600" />
			</div>
			<div class="space-y-1">
				<h3 class="text-lg font-black text-slate-900">Enrollment Failed</h3>
				<p class="text-sm text-slate-500">
					<span class="font-bold text-rose-700 capitalize">{selectedMethod}</span> enrollment could not
					be completed
				</p>
				<p class="mt-2 text-[10px] font-bold tracking-widest text-rose-400 uppercase">
					Error code: {failureCode}
				</p>
			</div>

			<div class="flex justify-center gap-3">
				<Button
					variant="outline"
					onclick={() => {
						step = categorySlug === 'card' ? 'card-input' : 'choose';
						failureCode = '';
					}}
					class="border-2 font-bold"
				>
					Try Again
				</Button>
				<Button variant="outline" onclick={handleSkip} class="border-2 font-bold text-slate-500">
					Close
				</Button>
			</div>
		</div>
	{/if}
</div>
